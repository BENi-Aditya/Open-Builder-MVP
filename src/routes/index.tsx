...
function FeedPage() {
  ...
  const load = async () => {
    ...
    const logQ = supabase.from("build_logs").select("id, body, image_url, created_at, project_id, project:projects(id, title), media:build_log_media(id, url, media_type, position), user:profiles!build_logs_user_id_fkey(id, username, display_name, avatar_url)").order("created_at", { ascending: false }).limit(15);
